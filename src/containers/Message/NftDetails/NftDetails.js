...
import { Checkbox, DialogActions, DialogContent, FormControlLabel, Menu, MenuItem, Typography } from "@material-ui/core";
...
import { validateInput, divideNo, isGreater, getNetworkByChainId, multipliedBy } from "../../../common/utils";
import { collectibleTypes, collectibleTexts } from "../../../data/tokenTypes";
import { useFetchTokens } from "../../../data/useFetchTokens";
import { SET_REPORT_NFTS, SET_USER_DATA } from "../../../redux/constants";
...
import { SIX_MONTHS_IN_SECONDS, FIFTEEN_DAYS_IN_SECONDS } from '../../../constants/expireTimes';
import AddChallengeSubmission from "../../Art/addChallengeSubmission";
import CommentsList from "../../Art/commentsList";
import OtherOwners from "../../Art/ownersList";
import "../../Art/style.scss";
import SubmissionsList from "../../Art/submissionsList";
import WinnersList from "../../Art/winnersList";
import { getBaseCurrencyPrice, getConversionRate, getTokenPrice } from "../../../common/validatePrice"
...
const TaskBody = ({ label = '', isUrl = false, isDescription = false, description, templateText }) => {
  return (
...
const NftDetails = (props) => {
  ...
  const bidService = new BidService(network.backendUrl);
  ...
  const womToEthPrice = useSelector(
    (state) => state.currencyReducer.womToEthPrice
  );
  ...
  const phyllo_accounts_connected = urlParams.get('phyllo_accounts_connected');
  const phyllo_id = urlParams.get('phyllo_id');

  // for report
  const reportNfts = useSelector((state) => state.profile.reportNfts);

  const reqBid = async (bids, accounts) => {
    let arr = [];
    let highestBid = { bidder: { price: 0 }, userData: "" };
    let bidderFound = false;
    for (let i = 0; i < bids.length; i++) {
      let userInfo = await userService.getUser(bids[i].bidder);
      if (userInfo) {
        arr.push({ bidder: bids[i], userData: userInfo });
      }
      if (accounts && bids[i].bidder.toLowerCase() === accounts.toLowerCase()) {
        setIsBidPlaced(true);
      }

      if (parseFloat(bids[i].price) >= parseFloat(highestBid.bidder.price)) {
        highestBid = { bidder: bids[i], userData: userInfo };
        bidderFound = true;
      }
    }
    setBiddersData(arr);
    bidderFound ? setHighestBidder(highestBid) : setHighestBidder("");
  };
  ...
  async function loadNftData() {
    ...
    try {
      let item = await nftService.getNft(props.match.params.nftId);
      if (item) {
        ...
        if (
          metaMaskAddress &&
          metaMaskAddress.toLowerCase() === item.holder.toLowerCase()
        ) {
          setIsOwner(true);
        } else {
          setIsOwner(false);
        ...
        if (item.nftType === "nft_challenge") {
          ...
          const challenge = await challengeService.getChallengeByAssetId(item.assetId);
          printLog(['challenge', challenge], 'success');
          if (challenge) setChallenge(challenge);
        }
        nftService.addViews(item.id);
        let userRelatedNftData = await nftService.userNFTData(
          item.holderData.address
        );
        if (userRelatedNftData) {
          printLog([userRelatedNftData], 'success');
  ...
  async function getDetailsAfterConnect() {
    if (nft) {
      ...
      if (nft.offers && nft.offers.length) {
        setLastOffer(nft.offers[0]);
        if (nft.offers[0].isForAuction) {
          let bids = nft.offers[0]?.bids;
          reqBid(bids, metaMaskAddress);
        } else {
          let bids = [];
          reqBid(bids, metaMaskAddress);
  ...
  // const checkNftAirdropped = async () => {
  //   if (nft?.nftType != 'nft_challenge' && nft?.holderData.address.toLowerCase() == userData?.address?.toLowerCase()) {
  //     let erc1155Challenge = getNFTChallengeInstance(
  //       web3Object,
  //       "ERC1155",
  //       network
  //     );
  //     let result = await erc1155Challenge.getIfNftAirdropped(nft.collectionType.address, nft.assetId);
  //     setShowChallengeButton(!result);
  //   }
  // }
  ...
  useEffect(() => {
    ...
    // setting task list
    setTaskList(_.groupBy(createTasks, 'social_name'))
  }, []);
  ...
  const cancelBid = async () => {
    ...
    let userBid = biddersData.find(
      (obj) => obj.bidder.bidder.toLowerCase() === metaMaskAddress.toLowerCase()
    );
    if (lastOffer && acccouts[0] && userBid) {
      try {
        printLog([userBid], 'success');
        let bid = userBid.bidder;
        setLoading(true);
        ...
        const connectedWith = localStorage.getItem("connectedWith");
        const eventIdBidCancelled = (Math.floor((Date.now() + Math.random()) * 10000)).toString();

        const cancelBidCallback = async (success, data) => {
          if (success) {
            const endRequestsAt = Date.now() + parseInt(process.env.REACT_APP_TRANSACTION_WAITING_TIME);
            const verified = await verifyTransaction(tempStorageService, eventIdBidCancelled
              , endRequestsAt);
            if (verified) {
              loadNftData();
              setLoading(false);
              setSuccessModalText("You successfully cancelled the bid!")
        ...
        const tempobj = {
          eventId: eventIdBidCancelled,
          event_name: 'BidCancelled',
          user_id: bid.bidder,
  ...
  const cancelOffer = async () => {
    printLog(['cancelOffer clicked'], 'success');
    if (!checkNetwork()) {
      return;
    }

    if (Number(lastOffer.expiresAt) > (new Date()).getTime()) {
      showNotification("Cancel Offer is applicable after expire time.");
      return;
    }
    try {
      setLoading(true);
      let nftExchanger = getNFTExchangerInstance(
        web3Object,
        nft.contractType,
        network,
        wcPopupConfig,
      ...
      const connectedWith = localStorage.getItem("connectedWith");
      const eventIdCancelOffer = (Math.floor((Date.now() + Math.random()) * 10000)).toString();
      const cancelOfferCallback = async (success, data) => {
        if (success) {

          const endRequestsAt = Date.now() + parseInt(process.env.REACT_APP_TRANSACTION_WAITING_TIME);
          const verified = await verifyTransaction(tempStorageService, eventIdCancelOffer, endRequestsAt);
          if (verified) {
            // await deleteOffer(lastOffer.id);
            loadNftData();
            setLoading(false);
            setSuccessModalText("You successfully cancelled the offer!")
            setPurchaseSuccessModalOpen(true);
          } else {
            setLoading(false);
            showNotification('Transaction timeout, but still your transaction could be successful.');
          }
        } else {
          setLoading(false);
        }
      ...
      const tempobj = {
        eventId: eventIdCancelOffer,
        event_name: 'CancelOffer',
        user_id: lastOffer.seller,
        json_string: JSON.stringify(tempDataObj)
      }

      const tempRes = await tempStorageService.save(tempobj);
      if (tempRes) {
        await nftExchanger.cancelOffer(
          lastOffer.seller,
          nft.collectionType.address,
          nft.assetId,
          lastOffer.offer_id,
  ...
  const renewOffer = async () => {
    ...
    try {
      ...
      const newExpireTime = lastOffer.isForAuction ? currentTime + FIFTEEN_DAYS_IN_SECONDS : currentTime + SIX_MONTHS_IN_SECONDS;
      let updatedOffer = {}
      let nftExchanger = getNFTExchangerInstance(
        web3Object,
        nft.contractType,
      ...
      const connectedWith = localStorage.getItem("connectedWith");
      const eventIdSetExpireAt = (Math.floor((Date.now() + Math.random()) * 10000)).toString();

      const renewOfferCallback = async (success, data) => {
        if (success) {
          const endRequestsAt = Date.now() + parseInt(process.env.REACT_APP_TRANSACTION_WAITING_TIME);
          const verified = await verifyTransaction(tempStorageService, eventIdSetExpireAt, endRequestsAt);
          if (verified) {
            loadNftData();
            setLoading(false);
          } else {
      ...
      const tempDataObj = {
        id: lastOffer.id,
        expiresAt: (newExpireTime * 1000),
      };

      const tempobj = {
        eventId: eventIdSetExpireAt,
        event_name: 'SetExpireAt',
        user_id: lastOffer.seller,
        json_string: JSON.stringify(tempDataObj)
      }
      ...
      const tempRes = await tempStorageService.save(tempobj);
      if (tempRes) {
        nftExchanger.setExpiresAt(
          lastOffer.seller,
          nft.collectionType.address,
          nft.assetId,
          newExpireTime,// is for sell
          lastOffer.offer_id,
          eventIdSetExpireAt,
          connectedWith,
          renewOfferCallback
  ...
  const prepareBid = async () => {
    if (!lastOffer || !validateInput(price)) {
      ...
      showNotification("Please enter the link.");
      return;
    } else if (price <= lastOffer.price) {
      showNotification(
        `Price should be greater than ${lastOffer.price} ${lastOffer.currency}`
      );
      return;
    }
    ...
    if (
      ...
      (!nftSupply || parseFloat(nftSupply) > parseFloat(nft.supply))
    ) {
      showNotification("Please enter the valid nft amount to bid");
      return;
    }

    const share = await erc721Exchanger.getShareByIndex(nft.shareIndex);

    let shareValue = new Big(parseFloat(share) / 100);
    let nft_price = new Big(price);
    let nft_supply = new Big(nftSupply);
    let totalPriceWithSupply = nft_price.times(nft_supply);
    let totalPriceWithShare = totalPriceWithSupply.times(shareValue);
    let total = totalPriceWithSupply.plus(totalPriceWithShare);
    printLog([total.toString()], 'success');

    let priceAfterShare = total.toString();

    setTotalPrice(priceAfterShare);
    setConfirmBid(true);
  };
  const getTokenAddress = (symbol) => {
    for (let token of tokens) {
      if (token.currencyKey.toLowerCase() === symbol.toLowerCase())
        return token.value;
    }
  }
  const getTokenName = (address) => {
    for (let token of tokens) {
      if (token.value.toLowerCase() === address.toLowerCase())
        return token.currencyKey;
    }
  }
  const placeBid = async (nft) => {
    ...
    setLoading(true);
    const acccouts = await web3Object?.eth?.getAccounts();
    if (lastOffer && acccouts[0]) {
      const bidder = acccouts[0];
      let tokenCoin = getTokenAddress(lastOffer.currency);
      printLog(["token", tokenCoin], 'success');
      let erc721Exchanger = getNFTExchangerInstance(
        web3Object,
        nft.contractType,
        network,
      ...
      const erc20Exchanger = new ERC20Exchanger(web3Object, tokenCoin, wcPopupConfig, dispatch);
      const decimals = await erc20Exchanger.decimals();
      // let decimalValue = new Big("10").pow(parseInt(decimals));
      // const strPrice = new Big(totalPrice).times(decimalValue).toString();
      const strPrice = multipliedBy(totalPrice, decimals);
      printLog(["strPrice", strPrice], 'success');

      //get balance
      const balance = await erc20Exchanger.balanceOf(acccouts[0]);
      const actualBalance = divideNo(balance, decimals);
      const actualPrice = divideNo(strPrice, decimals);
      const connectedWith = localStorage.getItem("connectedWith");

      if (isGreater(actualPrice, actualBalance)) {
        showNotification(
          "You can place bid with price less or equal to your balance"
        );
        setLoading(false);
        return;
      }
      ...
      const spenderAddress = nft.contractType === "ERC1155"
        ? network.smartContracts.ERC1155_EXCHANGER_ADDRESS
        : network.smartContracts.ERC721_EXCHANGER_ADDRESS;

      let allowance = await erc20Exchanger.getAllowance(bidder, spenderAddress);
      printLog(['allowance', allowance], 'success');
      const eventIdBidCreated = (Math.floor((Date.now() + Math.random()) * 10000)).toString();
      const placeBidCallback = async (success, data) => {
        if (success) {
          const endRequestsAt = Date.now() + parseInt(process.env.REACT_APP_TRANSACTION_WAITING_TIME);
          const verified = await verifyTransaction(tempStorageService, eventIdBidCreated
            , endRequestsAt);
          if (verified) {
            loadNftData();
            ...
            setConfirmBid(false);
            setBidDialogOpen(false);
            setSuccessModalText("You have successfully submitted your bid for the NFT. Discover more NFTs!")
            setPurchaseSuccessModalOpen(true);
          } else {
            setLoading(false);
          }
      ...
      const allowanceCallback = async (success, data) => {
        if (success) {
          const tempDataObj = {
            ...
            currency: lastOffer.currency ? lastOffer.currency : "ETH",
            supply: nft.contractType === "ERC1155" ? nftSupply : 0,
            fileUrl: fileUrl,
          }

          const tempobj = {
            eventId: eventIdBidCreated,
            event_name: 'BidCreated',
      ...
      } else if (Number(allowance) >= Number(strPrice)) {
        printLog(['step 2'], 'success');
        allowanceCallback(true);
      } else {
        printLog(['step 3'], 'success');
        let allowanceIncreaseValue = Number(strPrice) - allowance;
  ...
  const prepareBuy = async (nft) => {
    if (
      nft?.contractType === "ERC1155" &&
      (!nftSupply || parseFloat(nftSupply) > parseFloat(nft.supply))
    ) {
      showNotification(`You can't buy more than ${nft.supply} copy.`);
      return;
    } else if (
      (nft.nftType === "tiktok_duet" ||
        nft.nftType === "tiktok_collab" ||
    ...
    let shareValue = new Big(parseFloat(share) / 100);
    let nft_price = new Big(nft.offers[0].price);
    let nft_supply = new Big(nftSupply);
    let totalPriceWithSupply = nft_price.times(nft_supply);
    setSellerAmount(totalPriceWithSupply);
    let totalPriceWithShare = totalPriceWithSupply.times(shareValue);
    setAdminFee(totalPriceWithShare);
    let total = totalPriceWithSupply.plus(totalPriceWithShare);
    printLog([total.toString()], 'success');
    let priceAfterShare = total.toString();
    setTotalPrice(priceAfterShare);
  ...
  const buyNFT = async () => {
    ...
    const connectedWith = localStorage.getItem("connectedWith");
    const eventIdSwapped = (Math.floor((Date.now() + Math.random()) * 10000)).toString();
    const buyOfferCallback = async (success, data) => {
      if (success) {
        const endRequestsAt = Date.now() + parseInt(process.env.REACT_APP_TRANSACTION_WAITING_TIME);
        const verified = await verifyTransaction(tempStorageService, eventIdSwapped
          , endRequestsAt);
        if (verified) {
          loadNftData();
          setConfirmBuy(false);
          setLoading(false);
          setBuyNowDialogOpen(false);
          setSuccessModalText("You have successfully purchased the NFT. Go check it out now!")
          setPurchaseSuccessModalOpen(true);
        } else {
          setConfirmBuy(false);
          setLoading(false);
    ...
    //calculate royalty fee and admin fee
    let creatorBenif = 0;
    if (nft.resale) {
      if (nft.creator.toLowerCase() != nft.holderData.address.toLowerCase()) {
        const percent = nft.loyaltyPercentage
        if (percent > 0) {
    ...
    const basaePrice = await getBaseCurrencyPrice(network);

    const tempDataObj = {
      nftId: nft.id,
      buyerAddress: newHolder.toLowerCase(),
      nftSupply: nftSupply,
      totalPrice: totalPrice,
      sellerAddress: nft.holderData.address,
      offerId: nft.offers[0].id,
      royalty: creatorBenif,
      royalty_token: '',
      royalty_currency: network.nativeCurrency.symbol,
      royalty_usd: parseFloat(basaePrice) * creatorBenif,

      admin_fee_token: '',
      admin_fee_currency: network.nativeCurrency.symbol,
      admin_fee: adminFee,
      admin_fee_usd: parseFloat(basaePrice) * adminFee
    }
    const tempobj = {
      eventId: eventIdSwapped,
      event_name: 'Swapped',
      user_id: newHolder.toLowerCase(),
      json_string: JSON.stringify(tempDataObj)
    }
    const tempRes = await tempStorageService.save(tempobj);
    if (tempRes) {
      if (nft.contractType === "ERC1155") {
        try {
          erc721Exchanger.buyOffer(
            newHolder,
            nft.collectionType.address, //collection address
            nft.offers[0].offer_id,
            web3Object.utils.toWei(totalPrice.toString(), "ether"),
            parseInt(nftSupply),
            eventIdSwapped,
            connectedWith,
            buyOfferCallback
        ...
        nft.holder = newHolder.toLowerCase();
        erc721Exchanger.buyOffer(
          newHolder,
          nft.collectionType.address, //collection address
          nft.assetId,
  ...
  const acceptBid = async (bid) => {
    printLog([bid], 'success');
    if (!checkNetwork()) {
      return;
    }

    if (parseInt(nft.supply) < parseInt(bid.supply)) {
      showNotification("You don't have enough amount to accept that bid.");
      return;
    }
    // const priceConversionRate = womToEthPrice; //1 WOM = 0.001 ETH
    const acccouts = await web3Object.eth.getAccounts();
    if (lastOffer && isUserOwner && acccouts[0]) {
      const priceConversionRate = await getConversionRate(lastOffer.currency, network);
      printLog(['priceConversionRate', priceConversionRate], 'success');
      setLoading(true);
      ...
      printLog([bid.bidder], 'success');
      const connectedWith = localStorage.getItem("connectedWith");
      const eventIdBidSuccessful = (Math.floor((Date.now() + Math.random()) * 10000)).toString();
      const acceptBidCallback = async (success, data) => {
        if (success) {
          const endRequestsAt = Date.now() + parseInt(process.env.REACT_APP_TRANSACTION_WAITING_TIME);
          const verified = await verifyTransaction(tempStorageService, eventIdBidSuccessful
            , endRequestsAt);
          if (verified) {
            loadNftData();
            setLoading(false);
            setSuccessModalText("You successfully accepted the bid!")
            setPurchaseSuccessModalOpen(true);
          } else {
            setLoading(false);
          }
      ...
      const _totalPrice = parseFloat(bid.price);
      const share = await erc721Exchanger.getShareByIndex(nft.shareIndex);

      const _sellerAmount = (_totalPrice * 100) / (parseFloat(share) + 100);
      const _adminFee = _totalPrice - _sellerAmount;

      let creatorBenif = 0;
      if (nft.resale) {
        if (nft.creator.toLowerCase() != nft.holderData.address.toLowerCase()) {
          const percent = parseFloat(nft.loyaltyPercentage);
          if (percent > 0) {
            creatorBenif = _sellerAmount * percent / 100;
          }
        }
      }

      const _currency = getTokenName(bid.token);
      const _basaePrice = await getTokenPrice(_currency, network);

      const tempDataObj = {
        nftId: nft.id,
        bidderAddress: bid.bidder.toLowerCase(),
        bidSupply: bid.supply,
        bidPrice: bid.price,
        sellerAddress: nft.holderData.address,
        offerId: nft.offers[0].id,
        ...
        royalty: creatorBenif,
        royalty_token: bid.token,
        royalty_currency: _currency,
        royalty_usd: parseFloat(_basaePrice) * creatorBenif,

        admin_fee_token: bid.token,
        admin_fee_currency: _currency,
        admin_fee: _adminFee,
        admin_fee_usd: parseFloat(_basaePrice) * _adminFee
      }
      const tempobj = {
        eventId: eventIdBidSuccessful,
        event_name: 'BidSuccessful',
        user_id: owner.toLowerCase(),
        json_string: JSON.stringify(tempDataObj)
      }
      const tempRes = await tempStorageService.save(tempobj);
      if (tempRes) {
        erc721Exchanger.acceptBid(
          owner,
          nft.collectionType.address, //collecion address
          nft.assetId,
          bid.bidder,
          lastOffer.offer_id,
  ...
  const onNumberFieldChange = (e) => {
    ...
    if (e.target.name === "price" && reForDecimalNo.test(e.target.value)) {
  ...
  const changeNFTSupply = (e) => {
    ...
    if (
      ...
      if (e.target.value > lastOffer.supply) {
        showNotification("Not enought supply.");
      } else {
  ...
  const handleBuyNowDialogOpen = () => {
    if (!isWeb3Connected) {
      ...
      return;
    } else if (!lastOffer || (lastOffer && !lastOffer.isForSell)) {
      showNotification("NFT has not been put on sale.");
      return;
    } else if (isUserOwner) {
      showNotification("Owner/Holder can not buy his own nft.");
      return;
    } else if (lastOffer && lastOffer.isForAuction) {
      showNotification("NFT’s on Auction can not be bought. Please Place a Bid.");
      return;
    }
  ...
  const handleButtonClick = () => {
    if (lastOffer.auctionStartTime > new Date().getTime()) {
      showNotification("Auction not been started yet.");
      return;
    }
  ...
  const handleBidDialogOpen = () => {
    if (!isWeb3Connected) {
      ...
      showNotification("Owner/Holder can not buy his own nft.");
      return;
    } else if (lastOffer && !lastOffer.isForAuction && lastOffer.isForSell) {
      showNotification("You can not bid NFT on direct sell.");
      return;
    }
  ...
  const getNameAndIcon = (type) => {
    for (let i = 0; i < collectibleTypes.length; i++) {
      if (type === collectibleTypes[i].type) {
        return collectibleTypes[i];
      }
    }
  };
  ...
  const handleAddSubmission = () => {
    ...
    if (submissions && submissions.length === challenge.submissionLimit) {
      showNotification(
    ...
    if (new Date().getTime() >= new Date(parseInt(challenge.expiresAt)).getTime()) {
      showNotification("Submission time has been ended.");
      return;
    }

    if (isTwitterNeeded || isInstagramNeeded) {
  ...
  useEffect(() => {
    if (challenge) getSubmissionsList();
  }, [challenge]);
  ...
  const handleReportSubmit = async () => {
    if (Object.values(reportValues).find(item => item === true)) {
      if (!reportNfts.includes(nft.id)) {
        ...
        if (addReport) {
          dispatch({
            type: SET_REPORT_NFTS,
            payload: [...reportNfts, nft.id],
          })
        ...
        showNotification("you already submitted report for this NFT.");
      }
    } else {
  ...
  const handleNftTypeClick = (type) => {
    if (type) {
      history.push(`/explorer/${type}`);
    }
  }

  const viewChallenge = async () => {
    const url = `/token-challenge/${challenge.id}`;
    history.push({ pathname: url, state: { challenge: challenge, nft: nft } });
  }
  ...
  const renderNetwork = (chainId) => {
    ...
    return <p className="network-name">
      <CustomTooltip title={`This NFT Challenge is on ${nftNetwork?.chainName} blockchain`} placement="top-start">
        <>
          {icon}
        </>
      </CustomTooltip>
  ...
  const hadleOnShareClick = async (type) => {
    let extUrl = "";
    if (type === "twitter") {
      extUrl = `http://twitter.com/share?text=Check this awesome NFT on BULLZ&url=${url}`;
    } else if (type === "facebook") {
      extUrl = `https://www.facebook.com/sharer.php?display=page&u=${url}`
    } else if (type === "telegram") {
      extUrl = `https://telegram.me/share/url?url=${url}&text=Check out this awesome NFT on BULLZ`;
  ...
  return (
    <>
      {nft ? (
        <div className="detail-page nft-details">
          <div className="container">
            ...
            >
              <div className="h-100">
                <div className="page-content h-100 position-relative">
                ...
                <div className="favorite-wrapper align-items-center">
                          {nft?.holderData?.address?.toLowerCase() == userData?.address?.toLowerCase() && 
                              <CustomTooltip title='Refresh metadata' placement="top">
                                <img 
                            ...
                            className="mr-2" style={{height: 'auto', width: 'auto'}} width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className="more-options">
                    ...
                    >
                      ...
                      <MenuItem className="more-item" onClick={() => {
                        if(isWeb3Connected){
                          setIsMoreOpen(true)
                          handleMoreOptionClose();
                        } else {
                  ...
                  <div className="left-container desktop">
                    ...
                    {/* <div className="others-container">
                      ...
                      {selectedTab === "owners" ? (
                        <OtherOwners nft={nft} />
                      ) : selectedTab === "bids" && biddersData.length > 0 ? (
                          ...
                          return (
                            <div className="owner-item" key={index}>
                              <div className="owner-user">
                                <img
                                  className="profile"
                                  src={bid.userData.avatar_img}
                                  alt="User"
                                  onClick={() => history.push(`/profile/${bid.userData.address}`)}
                                />
                                <div className="owner-info">
                                  <p className="owner-name">
                                    ...
                                    >
                                      {bid.bidder.price}{" "}
                                      {getTokenName(bid.bidder.token)}
                                    </strong>
                                    <strong onClick={() => history.push(`/profile/${bid.userData.address}`)}>
                                      {" "}
                                      by @{bid.userData.username}
                                    </strong>{" "}
                                  </p>
                                  {nft.contractType === "ERC1155" ? (
                                    <p className="owner-lead">
                                      Amount: &nbsp;{bid.bidder.supply}
                                    </p>
                                  ) : undefined}
                                </div>
                              </div>
                              {isUserOwner && isWeb3Connected && (
                                <button
                                  className="btn-buy"
                                  onClick={() => acceptBid(bid.bidder)}
                                >
                                  Accept
                                </button>
                              )}
                        ...
                        <WinnersList nft={nft} challenge={challenge} />
                      ) : selectedTab === "comments" ? (
                        <CommentsList
                          nft={nft}
                  ...
                  <div className="detail-wrapper new with-bottom-bar">
                    ...
                    <div className="detail-new-content">
                      <div className="detail-new-content-header">
                        ...
                        <div className="header-item">
                          {renderNetwork(nft.chain_id)}
                          {nft.status == 2 && <p className="network-name">
                      ...
                      <div className="details-new-content-stat">
                        <div className="d-flex align-items-center">
                          <img className="avatar" src={nft?.creator?.avatar_img ? nft?.creator?.avatar_img : "/images/default-profile-cover.png"} alt="" />
                          <span className="username">@{nft.holderData.username}</span>
                        </div>
                      ...
                      <div className="left-container nft-details">
                        <div className="others-container">
                          <div
                            className={`custom-switch-2`}
                            style={{ width: "99%", padding: 0 }}
                      ...
                      {/* <div className="task-container">
                        {printLog(["selectedTask", selectedTask], 'success')}
                        {
                              ...
                              <label>{key} <InfoIcon /></label>
                              {
                                item.map(innerItem => (
                                  <CustomAccordion
                                    ...
                                    title={innerItem.task_name}
                                    handleChange={() => handleTaskSelect(innerItem.id)}>
                                    <TaskBody
                                      key={innerItem.id}
                                      description="This is test description"
                                      templateText={"This is test templatetext"}
                                      isDescription={true}
                                      isUrl={true}
                                      label={innerItem.task_name}
                                    />
                                  </CustomAccordion>
                                ))
                              }
            ...
            <div className="more-options">
                    ...
                    >
                      ...
                      <p className="content-lead">{parse(nft?.description ? nft.description : "")}</p>
                      <div className="d-flex flex-wrap mb-3">
                                    ...
                                    >
                                      <div className="styled-icon">
                                        <svg width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                                          <path d="M16.5041 11.0019V17.0035C16.5041 17.5341 16.2933 18.043 15.9181 18.4181C15.543 18.7933 15.0341 19.0041 14.5035 19.0041H3.50054C2.96997 19.0041 2.46112 18.7933 2.08595 18.4181C1.71077 18.043 1.5 17.5341 1.5 17.0035V6.00054C1.5 5.46997 1.71077 4.96112 2.08595 4.58595C2.46112 4.21077 2.96997 4 3.50054 4H9.50218" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                          <path d="M13.5029 1H19.5046V7.00163" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                          <path d="M8.5011 12.003L19.5041 1" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                        </svg>
                                      </div>
                                    </a>
                                  </CustomTooltip>
                        ...
                        {nft.instagramUrl && (
                          <div className="collection-item">
                            <p className="collection-lead">Instagram Link</p>
                            <div className="d-flex">
                              <a
                                href={getUrl(nft.instagramUrl)}
                                rel="noopener noreferrer"
                                target="_blank"
                              >
                                <InstagramIcon />
                        ...
                        {nft?.dFile && (
                          <div className="collection-item">
                            <p className="collection-lead">Shared File</p>
                            <div className="d-flex">
                              <a
                                href={getUrl(nft.dFile)}
                                rel="noopener noreferrer"
                                target="_blank"
                              >
                                <div className="styled-icon">
                          ...
                          {nft.resale ? (
                            <div className="styled-icon">
                              <svg width="23" height="17" viewBox="0 0 23 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M9.14151 17C8.49238 17 7.86644 16.7412 7.40277 16.2711L1.21288 9.99518C0.262373 9.03143 0.262373 7.45652 1.21288 6.49277C2.16339 5.52902 3.71666 5.52902 4.66717 6.49277L9.07196 10.9589L18.2525 0.804295C19.1566 -0.206467 20.7099 -0.276986 21.7068 0.663258C22.7036 1.58 22.7732 3.15491 21.8459 4.16567L10.9498 16.2008C10.5093 16.6942 9.86018 17 9.18787 17H9.14151Z" fill="white"/>
                              </svg>
              ...
              >
                <DialogContent>
                  <p className="main-title">Buy Now</p>
                  <p className="subtitle">You are about to buy</p>
                  <p className="subtitle blue-text">{nft.name}</p>
                      ...
                      <>
                        <div style={{ color: "white", marginBottom: 15 }}>
                          ...
                          <div>
                            <input
                              type={"text"}
                              className="bid-input text-center w-100"
                              value={fileUrl}
                  ...
                  <div className="input-group">
                    ...
                    <p className="info">Send transaction with your wallet.</p>
                  </div>
                  {nft.contractType === "ERC1155" && (
                    <>
                      <div className="input-group">
                      ...
                      <p className="info">Available Supply: {nft.supply}</p>
                    </>
                  )}
                </DialogContent>
                <DialogActions>
                  {confirmBuy && (
                    <p className="info">
                      Service fees : {nft.shareIndex}% &nbsp; Total price :{" "}
                      {totalPrice}
                    </p>
                  )}
                  <button
                  ...
                  >
                    {confirmBuy ? "Confirm buy" : "Buy Now"}
                  </button>
                  <button
                    className="btn-continue btn-cancel"
                    onClick={handleBuyNowDialogClose}
                  >
                    CANCEL
                  </button>
                </DialogActions>
              ...
              >
                <DialogContent>
                  <p className="main-title">Bid Now</p>
                  <p className="subtitle">You are about to bid for</p>
                  <p className="subtitle blue-text">{nft.name}</p>

                  <p className="info">
                    Minimum bid price: {lastOffer && lastOffer.price}{" "}
                    {lastOffer.currency ? lastOffer.currency : "ETH"}
                  </p>
                  <div className="input-group">
                    ...
                    <div className="input-wrapper">
                      ...
                      <div className="wom-select">
                        <p>{lastOffer.currency ? lastOffer.currency : "ETH"}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* {(nft.nftType === "tiktok_duet" ||
                    ...
                    nft.nftType === "music_promo") && (
                      <>
                        <div style={{ color: "white", marginBottom: 15 }}>
                          <div style={{marginBottom: 5}}>Enter tiktok video url</div>
                          <div>
                            <input
                              type={"text"}
                              className="bid-input text-center w-100"
                  ...
                  <div className="bid-item align-items-center">
                    <p className="buy-now-sub-lead mb-0">Place a Bid</p>
                    <div className="bid-action-wrapper">
                      <div className="form-wrapper">
                        <input
                          className="bid-input text-center"
                          value={price}
                          name={"price"}
                          onChange={onNumberFieldChange}
                        />{" "}
                  ...
                  {nft.contractType === "ERC1155" && (
                    <>
                      <div className="input-group">
                        <label>Amount to Buy(Max: {nft.supply})</label>
                        {confirmBid ? (
                          <input
                            disabled
                            className="new-bid-input"
                      ...
                      {/* <div className="bid-item align-items-center amnt-to-buy">
                      <CustomTooltip title={nft.supply} placement="top">
                        <p className="buy-now-sub-lead mb-0">Amount to Buy(Max: {nft.supply})</p>
                      </CustomTooltip>
                ...
                <DialogActions>
                  {!confirmBid && (
                    <button
                      className="btn-continue"
                      onClick={() =>
                        // isWeb3Connected ? prepareBid() : connectWallet()
                        isWeb3Connected ? prepareBid() : setIsConnectWalletAlertOpen(true)
                      }
                    >
                      Place a Bid
                    </button>
                  ...
                  {confirmBid && (
                    <>
                      <p className="info">
                        Service fees : {nft.shareIndex} % <br />
                        Total price : {totalPrice}
                      </p>
                      <button
                        onClick={() =>
                          // isWeb3Connected ? placeBid(nft) : connectWallet()
                          isWeb3Connected ? placeBid(nft) : setIsConnectWalletAlertOpen(true)
                        }
                        className="btn-continue"
                      >
                        Confirm bid
              ...
              >
                ...
                <DialogContent>
                  <div className="report-content">
                      ...
                      <FormControlLabel
                        control={
                          <Checkbox
                            ...
                            checked={reportValues?.doesNotFollowRequirements} name="doesNotFollowRequirements" onChange={handleReportCheckboxChange} />
                        }
                      ...
                      <FormControlLabel
                        control={
                          <Checkbox
                            ...
                            checked={reportValues?.copyright} name="copyright" onChange={handleReportCheckboxChange} />
                        }
                      ...
                      <FormControlLabel
                        control={
                          <Checkbox
                            ...
                            checked={reportValues?.poorQuality} name="poorQuality" onChange={handleReportCheckboxChange} />
                        }
                        label="Poor quality"
                      />
                    </div>
                      ...
                      <FormControlLabel
                        control={
                          <Checkbox
                            ...
                            checked={reportValues?.inappropriate} name="inappropriate" onChange={handleReportCheckboxChange} />
                        }
                        label="Inappropriate"
                      />
                    </div>
                      ...
                      <FormControlLabel
                        control={
                          <Checkbox
                            ...
                            checked={reportValues?.other} name="other" onChange={handleReportCheckboxChange} />
                        }
                        label="Other"
                      />
                    </div>
                    ...
                    <div className="actions">
                      <button
                        disabled={!Object.values(reportValues).find(item => item === true)}
          ...
          {lastOffer ? (
            <div className="bottom-action-section art-type">
              <div className={`container ${!(lastOffer && lastOffer.isForAuction) ? "justify-content-center" : ""}`}>
                {/* <div className="details"> */}
                {lastOffer && lastOffer.isForAuction && (<div className="details">
                  <div className="details-item">
                    {highestBidder && (<>
                      <p className="details-item-lead">
                        Hightest bid by{" "}
                        <span className="cursor-pointer" onClick={() => history.push(`/profile/${highestBidder.userData.address}`)}>
                          {lastOffer && lastOffer.isForAuction && highestBidder
                            ? '@' + highestBidder.userData.username
                            : "--"}
                        </span>
                      </p>
                      <p className="details-item-lead">
                        <span className="eth">
                          {lastOffer && lastOffer.isForAuction && highestBidder
                            ? highestBidder.bidder.price
                            : "--"}{" "}
                          {lastOffer && lastOffer.isForAuction && highestBidder
                            ? getTokenName(highestBidder.bidder.token)
                            : "--"}
                        </span>{" "}
                        for{" "}
                        {lastOffer && lastOffer.isForAuction && highestBidder
                          ? highestBidder.bidder.supply === 0
                            ? 1
                            : highestBidder.bidder.supply
                          : "--"}
                      </p>
                    </>)}
                  ...
                  <div className="details-item">
                    {isAuctionStarted(lastOffer.auctionStartTime) ? (
                      <>
                        <p className="details-item-lead">Auction Ending In</p>
                        <p className="details-item-lead">
                          {lastOffer && lastOffer.isForAuction ? (
                            <CountDown
                              auctionETime={parseInt(lastOffer.expiresAt)}
                            />
                          ) : (
                      ...
                      <>
                        <p className="details-item-lead">
                          Auction time was not started yet.
                        </p>
                      </>
                    )}
                  </div>
                ...
                <div className="challenge-actions">
                      ...
                      {userData?.address?.toLowerCase() ===
                        ...
                        lastOffer.isForSell || lastOffer.isForAuction ? (
                          ...
                          <>
                            <button
                              className="btn-continue active"
                              onClick={() => history.push(`/sellnft/${nft.id}`)}
                            >
                              Sell NFT
                            </button>
                        ...
                        <button className="btn-continue active">
                          Offer Expired
                        </button>
                      ) : (
                        <button className="btn-continue active">
                          ...
                          <>
                            {lastOffer ? (
                              lastOffer.isForSell && !lastOffer.isForAuction ? (
                                ...
                                >
                                  Buy for{" "}
                                  {lastOffer.price.toString().length > 8
                                    ? `${lastOffer.price
                                      .toString()
                                      .substring(0, 3)}...${lastOffer.price
                                        .toString()
                                        .substring(
                                          lastOffer.price.toString().length - 3,
                                          lastOffer.price.toString().length
                                        )}`
                                    : lastOffer.price}{" "}
                                  {lastOffer.currency}
                                </button>
                  ...
                  {currentTimeM <= lastOffer.expiresAt && userData?.address?.toLowerCase() !==
                    nft?.holderData.address?.toLowerCase() && (
                      <>
                        {lastOffer &&
                          lastOffer.isForSell &&
                          lastOffer.isForAuction && (
                            ...
                            >
                              {lastOffer && lastOffer.isForSell
                                ? isBidPlaced
                                  ? "Cancel Bid"
                                  : isAuctionStarted(lastOffer.auctionStartTime)
                                    ? `Place a Bid for ${lastOffer.price.toString().length > 6
                                      ? `${lastOffer.price
                                        .toString()
                                        .substring(0, 2)}...${lastOffer.price
                                          .toString()
                                          .substring(
                                            lastOffer.price.toString().length -
                                            2,
                  ...
                  {currentTimeM > lastOffer.expiresAt && (
                    <>
                      {userData?.address?.toLowerCase() ===
                        ...
                        lastOffer.isForSell || lastOffer.isForAuction ? (
                          <button
                            className="btn-continue inactive"
                            onClick={() => cancelOffer()}
                          >
                            Cancel Offer
                          </button>
                  ...
                  {/* <div className="btn-continue">Buy for 3.5 ETH</div>
                    <div className="btn-continue inactive">Place a Bid</div> */}
                </div>
              </div>
            </div>
          ) : (
                        ...
                        {!lastOffer && (
                          <>
                            {userData?.address?.toLowerCase() ===
                              nft?.holderData.address?.toLowerCase() && (
                                <>
                                  {(nft.resale || nft.holderData.address.toLowerCase() == nft.creatorData.address.toLowerCase()) ? (
                                    <>
                                      <button
                                        className="btn-continue active"
                                        onClick={() =>
                                          history.push(`/sellnft/${nft.id}`)
                                        }
                                      >
                                        Sell NFT
                                      </button>

                                      {/* <button className="btn-continue inactive" onClick={() => showNotification('Coming soon.')} >
                                    Open Offers
                                  </button> */}
                                    </>
                                  ) : (
                                    <>
                                      ...
                                      <button
                                        className="btn-continue"
                                        onClick={() => setShowTransferNFTPopup(true)}
                                      >
                                        Transfer NFT
                                      </button>
                                    </>
                            ...
                            {userData?.address?.toLowerCase() !==
                              nft?.holderData.address?.toLowerCase() && challenge && (
                                <button
                                  className="btn-continue active"
                                  onClick={() => viewChallenge()}
                                >
      ...
      >
        <DialogContent>
          ...
          <div className="congratulation-content">
            ...
            <p className="item-lead">You have successfully submitted your NFT Challenge. Go check it out now!</p>
            <button
              onClick={() => {
      ...
      >
        <DialogContent>
          ...
          <div className="congratulation-content">
            ...
            <p className="item-lead">There was a problem submitting to the NFT Challenge. Please try again or contact our support team. We are sorry for inconvenience.</p>
            <button
      ...
      {showTransferNFTPopup && (
        <TransferNft
          show={showTransferNFTPopup}
          setShowTransferNFTPopup={setShowTransferNFTPopup}
          hideShow={() => setShowTransferNFTPopup(false)}
      ...
      >
        <DialogContent>
          ...
          <div className="congratulation-content">
            ...
            {/* <p className="item-lead">
                Your {collectibleType.name} is live.
              </p>
              <p className="item-lead mb-3">Go check it out now.</p> */}
            <button
              onClick={() =>
      ...
      >
        ...
        <DialogContent>
          ...
          >
            ...
            <div className="d-flex flex-column align-items-center modal-share">
              <a
                href={` mailto:?to=&body=Check out this awesome NFT on BULLZ ${url}&subject=Check out this awesome NFT on BULLZ`}
                target="_blank"
                rel="noopener noreferrer"
                className="social-share-anchor"
              >
