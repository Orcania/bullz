...
const MyChallenge = (props) => {
  // let chainId = props?.history?.location?.state?.chainId;
  ...
  async function loadNfts() {
    ...
    if (!isFilterCleared && !networkType) 
      return;
    ...
    const response = await challengeService.getChallengesWithFilters(
  ...
  async function getSearchData(q) {
    ...
    if (q.length > 2) {
      setIsLoading(true);
      let res = await searchService.getSearchDataChallenge(q, selectedPage, limit);     
      await setData(res);
      setIsLoading(false);
    } else if (q.length == 0) {
      loadNfts();
  ...
  const  setData = async (response) => {
    ...
    if (response.data) {
      ...
      for (let i = 0; i < nftss.length; i++) {
        const _chalenge = nftss[i];
        if (_chalenge.asset_type == 1) {
          let item = _chalenge.nft;
          const holderData = _chalenge.creator;
      ...
      <div className="photo-profile-page challenge-page">
        <div className="profile-content">
          <div className="container position-relative">
            <div className="explore-heading challenges">
              <div className="d-flex step-crumb cursor-pointer" onClick={() => history.goBack()}>
                <KeyboardBackspaceIcon
                  style={{ color: "white", marginRight: 10 }}
                />
                <p className="step-lead">Challenges</p>
              </div>
              <p className="challenge-title challenge">Challenges</p>
              {isItem ? (
                <div
                  style={{
                    width: "100%",
                ...
                <>
                  {/* <p className="challenge-desc">
                    Please select NFT Challenge to add the submissions.
                  </p> */}
              ...
              isShowingFilter && (
                ...
                <div className="filter-bar-wrapper" ref={filterRef}>
                  ...
                  {(!isMobile || isMobileFilter) && (
                    <div className="filter-bar" style={{height: isMobile && (sortByOpen || statusOpen) ? 235 : 'auto'}}>
