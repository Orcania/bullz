...
function OwnersComponent({ nft, setSelectedNft, ...props }) {
  ...
  const getComments = async () => {
    const comm = await commentService.getNFTComments(nft.id, loggedInUser.id);
    if (comm) {
      let temp = [];
    ...
    if (isWeb3Connected) {
      if (comments && comment) {
        ...
        if (res) {
          ...
          const updatedNft = {
            id: nft.id,
            totalCommentCount: nft.totalCommentCount
              ? nft.totalCommentCount + 1
              : 1,
          };
          await nftService.updateNFT(updatedNft);
          setSelectedNft((prevState) => ({
            ...prevState,
            totalCommentCount: prevState.totalCommentCount + 1,
          }));
        }
      }
    } else {
  ...
  return (
    <React.Fragment>
      ...
      <CustomAlert type="" width={514} isDialogOpen={isConnectWalletAlertOpen} title="You're not logged in" message={"Please log in or sign up to post comment"} btnText="Connect Wallet" customDialogAlertClose={()=> setIsConnectWalletAlertOpen(false)} doSomething={handleConnectWallet}/>
    </React.Fragment>
  );
}
